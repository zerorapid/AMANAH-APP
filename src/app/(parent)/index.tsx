    ...
    <SafeAreaView style={s.container} edges={['top']}>
      <ScrollView contentContainerStyle={s.scroll} showsVerticalScrollIndicator={false}>
        ...
        <XStack justifyContent="space-between" alignItems="center" marginBottom={20}>
          <XStack gap={12} alignItems="center">
            ...
            <YStack>
              ...
              <Paragraph fontSize={17} fontWeight="bold" color={C.orange}>{parent.name}</Paragraph>
            </YStack>
          </XStack>
        ...
        <View style={s.balanceCard}>
          ...
          <Paragraph style={s.balAmount}>${parent.balance.toFixed(2)}</Paragraph>
          <View style={s.divider} />
          <XStack justifyContent="space-between">
            <YStack gap={2}>
              <Paragraph style={s.balSub}>Total Allocated</Paragraph>
              <Paragraph style={s.balVal}>${children.reduce((s, c) => s + c.balance, 0).toFixed(2)}</Paragraph>
            </YStack>
        ...
        <XStack gap={10} marginBottom={20}>
          {[{ label: 'Send', icon: <Send size={14} color={C.orange} /> },
            { label: 'Add Funds', icon: <Plus size={14} color={C.orange} /> },
            { label: 'Top Up', icon: <ArrowDownCircle size={14} color={C.orange} /> }
          ].map((a) => (
            ...
            {pending.map((r) => (
              <XStack key={r.id} justifyContent="space-between" alignItems="center" marginBottom={8}>
                <YStack flex={1}>
                  ...
                  <Paragraph fontSize={12} color={C.muted}>${r.amount.toFixed(2)} · {r.createdAt}</Paragraph>
                </YStack>
                <XStack gap={8}>
        ...
        <XStack justifyContent="space-between" alignItems="center" marginBottom={12}>
          ...
          <Button size="$2" backgroundColor={C.orangeLight} color={C.orange} icon={<Plus size={13} />}>Add</Button>
        </XStack>
          ...
          {children.map((child) => (
            <View key={child.id} style={s.card}>
              <XStack gap={12} alignItems="center" marginBottom={10}>
                ...
                <YStack flex={1}>
                  <Paragraph fontSize={15} fontWeight="bold" color={C.orange}>{child.name}</Paragraph>
                  <Paragraph fontSize={13} color={C.muted}>Balance: ${child.balance.toFixed(2)}</Paragraph>
                </YStack>
                <Button size="$2" backgroundColor={C.orangeLight} color={C.orange}>Manage</Button>
              </XStack>
              <XStack justifyContent="space-between" marginBottom={4}>
                <Paragraph fontSize={12} color={C.muted}>Monthly spending</Paragraph>
                <Paragraph fontSize={12} color={C.muted}>${child.spent} / ${child.monthlyLimit}</Paragraph>
              </XStack>
              <Progress value={(child.spent / child.monthlyLimit) * 100} size="$1" backgroundColor={C.orangeLight}>
                <Progress.Indicator backgroundColor={C.orange} />
              </Progress>
            </View>
          ))}
        </YStack>
...
const s = StyleSheet.create({
  ...
  quickBtn: { flex: 1, backgroundColor: C.orangeLight, borderRadius: 12, padding: 12,
    alignItems: 'center', gap: 4 },
  quickLabel: { color: C.orange, fontSize: 12, fontWeight: '600' },
